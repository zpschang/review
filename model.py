...
class Affect_LM_Model():
    ...
    def update(self, sess, beta, reader):
        ...
        if batch == None:
            ...
            batch = reader.get_batch(self.batch_size)
        # build feed dict
        feed_truth, feed_weight, feed_feature = self.process_batch(batch, reader, self.max_length)
        feed_dict = {}
    ...
    def inference(self, sess, beta, prefix_size, fixed_feature, reader, file):
        ...
        feed_prefix, feed_weight, feed_feature = self.process_batch(batch, reader, prefix_size)
